...
//#region app/pages/dashboard/service-requests.vue?vue&type=script&setup=true&lang.ts
var service_requests_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	__name: "service-requests",
	__ssrInlineRender: true,
	setup(__props) {
		useHead$1({ title: "Service Requests - Nairobi Powerbikes" });
		const pb = usePB();
		...
		async function loadData() {
			try {
				const res = await pb.collection("service_bookings").getList(1, 100, { sort: "-created" });
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "mx-auto max-w-7xl" }, _attrs))}><div class="mb-6"><h1 class="font-heading text-4xl text-white">Service <span class="text-brand-red">Requests</span></h1><p class="mt-1 text-sm text-brand-grey">Manage customer service requests</p></div>`);
			if (unref(loading)) {
				...
				_push(`<p class="mt-4 font-display text-xl tracking-display text-brand-grey">No Service Requests</p></div>`);
			} else {
				_push(`<div class="space-y-4"><!--[-->`);
				ssrRenderList(unref(items), (s) => {
					_push(`<div class="rounded-sm border border-brand-grey/20 bg-brand-black/60 p-5"><div class="flex flex-wrap items-start justify-between gap-3"><div><h3 class="font-display text-lg tracking-display text-white">${ssrInterpolate(s.name || s.expand?.user?.name || "Guest")}</h3><p class="text-xs text-brand-grey">${ssrInterpolate(s.service_type)} · ${ssrInterpolate(s.motorcycle)}</p></div>`);
...
//#endregion
//#region app/pages/dashboard/service-requests.vue
var _sfc_setup = service_requests_vue_vue_type_script_setup_true_lang_default.setup;
service_requests_vue_vue_type_script_setup_true_lang_default.setup = (props, ctx) => {
	const ssrContext = useSSRContext();
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/dashboard/service-requests.vue");
	return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
var service_requests_default = service_requests_vue_vue_type_script_setup_true_lang_default;

export { service_requests_default as default };
//# sourceMappingURL=service-requests-ChO-br1j.mjs.map
