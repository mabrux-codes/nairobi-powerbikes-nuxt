  ...
  <div class="min-h-screen bg-brand-black pt-24">
    <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <motion.div :initial="{ opacity: 0, y: 40 }" :animate="{ opacity: 1, y: 0 }" :transition="{ duration: 0.6 }">
        <h1 class="font-heading text-4xl text-white sm:text-5xl lg:text-display-xl">Apparel</h1>
        <div class="mt-2 h-1 w-24 bg-brand-red" />
        <p class="mt-4 text-brand-grey">Ride in style with our premium apparel collection.</p>
      </motion.div>
      ...
      <div v-if="loading" class="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <div v-for="i in 8" :key="i" class="animate-pulse rounded-sm border border-brand-grey/10 p-4"><div class="aspect-[3/4] w-full rounded-sm bg-brand-grey/10" /><div class="mt-4 h-5 w-3/4 rounded bg-brand-grey/10" /><div class="mt-2 h-4 w-1/2 rounded bg-brand-grey/10" /></div>
      </div>
      <div v-else-if="filtered.length" class="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <NuxtLink v-for="(item, i) in filtered" :key="item.id" :to="`/apparel/${item.id}`">
          <motion.div class="group rounded-sm border border-brand-grey/10 bg-brand-black/60 overflow-hidden transition-all duration-300 hover:border-brand-red/40"
            ...
            <div class="aspect-[3/4] overflow-hidden bg-brand-black">
...
interface ApparelItem { id: string; name: string; category: string; description?: string; price: number; images?: string[] }

useHead({ title: 'Apparel - Nairobi Powerbikes', meta: [{ name: 'description', content: 'Shop premium motorcycle apparel in Nairobi. Jackets, helmets, boots, gloves, and more.' }] })

const pb = usePB()
const loading = ref(true); const apparel = ref<ApparelItem[]>([])
const categories = ['All', 'Jackets', 'Helmets', 'Boots', 'Gloves', 'Pants', 'Base Layers', 'Rain Gear', 'Casual', 'Other']
...
onMounted(async () => {
  try { apparel.value = await pb.collection('apparel').getFullList<ApparelItem>({ sort: 'name' }) }
  catch { apparel.value = [] }
