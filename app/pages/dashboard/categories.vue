...
const pb = usePB(); const loading = ref(true); const saving = ref(false); const deleting = ref(false)
...
async function saveItem() { saving.value = true; try { if (editingId.value) { await pb.collection('categories').update(editingId.value, form.value); toast.add({ type: 'success', title: 'Updated successfully' }) } else { await pb.collection('categories').create(form.value); toast.add({ type: 'success', title: 'Created successfully' }) }; closeModal(); await loadData() } catch (e: any) { toast.add({ type: 'error', title: 'Failed to save', message: e?.message || 'Something went wrong' }) } finally { saving.value = false } }
async function confirmDelete(c: any) { deleting.value = true; try { const ok = await confirmDlg.confirm({ title: 'Delete Category', message: `Are you sure you want to delete "${c.name}"? This cannot be undone.`, confirmText: 'Delete', confirmType: 'danger' }); if (ok) { await pb.collection('categories').delete(c.id); toast.add({ type: 'success', title: 'Deleted successfully' }); await loadData() } } catch (e: any) { toast.add({ type: 'error', title: 'Failed to delete', message: e?.message || 'Something went wrong' }) } finally { deleting.value = false } }
