<template>
  <div class="mx-auto max-w-7xl">
    <div class="mb-6">
      <h1 class="font-heading text-4xl text-white">Service <span class="text-brand-red">Bookings</span></h1>
      <p class="mt-1 text-sm text-brand-grey">Manage customer service bookings</p>
    </div>
    ...
    <div v-else class="space-y-4">
      <div v-for="s in items" :key="s.id" class="rounded-sm border border-brand-grey/20 bg-brand-black/60 p-5">
        <div class="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h3 class="font-display text-lg tracking-display text-white">{{ s.name || 'Guest' }}</h3>
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 overflow-y-auto" @click.self="showModal=false">
        <div class="w-full max-w-md rounded-sm border border-brand-grey/30 bg-brand-black p-6">
          ...
          <div class="mt-4 space-y-4">
            ...
            <div><label class="mb-1.5 block text-xs font-display tracking-display text-brand-grey uppercase">Cost (KSh)</label>
...
async function saveUpdate() { saving.value = true; try { const p: any = { status: updateForm.value.status, notes: updateForm.value.notes }; if (updateForm.value.cost) p.cost = parseFloat(updateForm.value.cost); await pb.collection('service_bookings').update(editingItem.value.id, p); toast.add({ type: 'success', title: 'Service booking updated successfully' }); showModal.value = false; await loadData() } catch (e: any) { toast.add({ type: 'error', title: 'Failed to update service booking', message: e?.message || 'Something went wrong' }) } finally { saving.value = false } }
async function loadData() { try { const res = await pb.collection('service_bookings').getList(1, 100, { sort: '-created', filter: 'type="service"' }); items.value = res.items as any[] } catch (e) { console.error(e) } finally { loading.value = false } }
