  ...
  <div>
    <motion.div :initial="{ opacity: 0, y: 26 }" :animate="{ opacity: 1, y: 0 }" :transition="{ duration: 0.5, ease: 'easeOut' }">
      <div class="relative overflow-hidden rounded-2xl border border-white/[0.08] bg-gradient-to-b from-white/[0.05] to-white/[0.01] p-8 backdrop-blur-xl">
        ...
        <div class="mb-7">
          <h1 class="font-heading text-4xl text-white">Welcome <span class="text-brand-red">Back</span></h1>
        ...
        <form @submit.prevent="handleLogin" class="space-y-5" novalidate>
          <div>
            ...
            <div class="relative">
              ...
              <Field name="email" v-slot="{ componentField, errorMessage }">
                <input v-bind="componentField" id="login-email" type="email" autocomplete="email" placeholder="you@example.com" class="input-field h-12 rounded-xl border-white/10 bg-white/[0.03] pl-11 focus:border-brand-red focus:ring-2 focus:ring-brand-red/25" :class="{ 'border-brand-red/70': errorMessage }" />
          ...
          <div>
            ...
            <div class="relative">
              ...
              <Field name="password" v-slot="{ componentField, errorMessage }">
                <input v-bind="componentField" id="login-password" type="password" autocomplete="current-password" placeholder="••••••••" class="input-field h-12 rounded-xl border-white/10 bg-white/[0.03] pl-11 pr-12 focus:border-brand-red focus:ring-2 focus:ring-brand-red/25" :class="{ 'border-brand-red/70': errorMessage }" />
                <p v-if="errorMessage" class="mt-1.5 text-xs text-brand-red">{{ errorMessage }}</p>
              </Field>
            </div>
          </div>
          ...
          <div class="flex items-center justify-between gap-4">
            <label class="flex cursor-pointer items-center gap-2.5 select-none">
              <input v-model="rememberMe" type="checkbox" class="h-4 w-4 accent-brand-red" aria-label="Remember me" />
              <span class="text-sm text-brand-grey">Remember me</span>
            </label>
          ...
          <Transition name="fade-slide">
            <div v-if="showForgot" class="rounded-xl border border-white/10 bg-white/[0.03] p-4">
              ...
              <div class="mt-3 flex gap-2">
                ...
                <Button :loading="resetting" size="sm" variant="secondary" @click="sendReset">Send</Button>
              </div>
              <p v-if="resetMsg" class="mt-2 text-xs" :class="resetOk ? 'text-green-400' : 'text-brand-red'">{{ resetMsg }}</p>
            </div>
          </Transition>
