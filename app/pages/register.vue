    ...
    <motion.div :initial="{ opacity: 0, y: 30 }" :animate="{ opacity: 1, y: 0 }" :transition="{ duration: 0.5 }" class="w-full">
      <div class="rounded-sm border border-brand-grey/20 bg-brand-black p-8">
        ...
        <form @submit.prevent="handleRegister" class="space-y-5">
          ...
          <button type="submit" :disabled="isSubmitting" class="btn-primary w-full justify-center disabled:opacity-50"><LoaderCircle v-if="isSubmitting" class="h-5 w-5 animate-spin" /><UserPlus v-else class="h-5 w-5" />{{ isSubmitting ? 'Creating Account...' : 'Create Account' }}</button>
        </form>
