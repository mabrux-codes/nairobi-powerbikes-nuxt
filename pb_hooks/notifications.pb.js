...
// --- notify admins when a customer submits a booking or test ride ---
// Test rides live in their own `test_rides` collection since the split, so
// there is one create hook per collection.
onRecordAfterCreateSuccess((e) => {
  ...
  utils.broadcastToRole(e.app, "admin", {
    ...
    message: (r.getString("name") || "A customer") + " booked a " + (r.getString("service_type") || "service") + " appointment",
...
// Registered for both collections (service_bookings + test_rides); the cache
// key is namespaced per collection so the same record id can never collide.
function registerStatusHooks(coll, isTestRide) {
  onRecordUpdateRequest((e) => {
    const utils = require(__hooks + "/lib/notif_utils.js")
    const newStatus = e.record.getString("status")
    let oldStatus = newStatus
    try {
      oldStatus = e.app.findRecordById(coll, e.record.id).getString("status")
    } catch (err) {
    ...
    if (auth && userId) {
      ...
      if (isAdmin || isOwner) {
        utils.cacheStatusChange(e.app, coll + "_" + e.record.id, {
          isTestRide,
          newStatus,
          userId,
  ...
  onRecordAfterUpdateSuccess((e) => {
    const utils = require(__hooks + "/lib/notif_utils.js")
    const data = utils.consumeStatusChange(e.app, coll + "_" + e.record.id)
    if (!data) {
      e.next()
      return
    }
  ...
  onRecordAfterUpdateError((e) => {
    const utils = require(__hooks + "/lib/notif_utils.js")
    utils.consumeStatusChange(e.app, coll + "_" + e.record.id)
    e.next()
  }, coll)
}

registerStatusHooks("service_bookings", false)
registerStatusHooks("test_rides", true)
